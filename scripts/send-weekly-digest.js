...
async function main() {
  ...
  const prompt = "Составь короткий еженедельный дайджест для владельца массажной студии по данным бизнеса: "
    ...
    + "НЕ добавляй заголовок или название студии в начале — сразу переходи к содержанию. "
    + "Не используй markdown-разметку (# и **) — это обычный текст для Telegram.";
  ...
  // Защита от дубля заголовка: срезаем ведущий markdown-заголовок, если AI его добавил
  reply = reply.replace(/^\s*#{1,6}[^\n]*\n+/, "").trim();
